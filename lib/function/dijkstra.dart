...
// lay trong database roi parse no ra
    // List<List<int>> d1 = [
    //   [1, 5],
    //   [2, 6]
    // ];
    ...
    //   for(int j = 0; j<2; j++) {
    //     addEdge(adj, d1[i][j], d1[i][j], distance(6, 1))
    //     addEdge(adj, d1[j][i], d1[i][j], distance(6, 1))
    //   }
    // }

    addEdge(adj, 6, 1, distance(6, 1));
    addEdge(adj, 1, 6, distance(6, 1));

    addEdge(adj, 1, 2, distance(2, 1));
    addEdge(adj, 2, 1, distance(2, 1));

    addEdge(adj, 2, 3, distance(2, 3));
    addEdge(adj, 3, 2, distance(2, 3));
...
// dem so luong diem
  ...
  Future<int> getDeskLengthONha() async {
    List<DeskElement> desk = [];
    final String response = await rootBundle.loadString('assets/nhatren.json');
    final Map<String, dynamic> database = await json.decode(response);
    List<dynamic> data = database["children"][1]["children"];
